// --------------------------------------------------------------------------------------------------------------------
// <copyright file="rankLinkXPath.cs" company="imbVeles" >
//
...
namespace imbWEM.Core.crawler.rules.active
{
    ...
    public class rankLinkXPath : ruleActiveBase
    {
        public rankLinkXPath(ISpiderEvaluatorBase __parent, int __scoreUnit = 100)
            : base("XPath Depth", "Tributes score unit [{0}] proportionaly to inverse link HtmlNode XPath depth, normalized by the highest XPath depth among target links", __scoreUnit,0, __parent)
        {
        ...
        {
            int depth = page.link.html.XPath.Replace("/", "\\").getPathParts().Count();
