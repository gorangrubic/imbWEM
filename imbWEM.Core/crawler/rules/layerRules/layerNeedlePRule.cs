// --------------------------------------------------------------------------------------------------------------------
// <copyright file="layerNeedlePRule.cs" company="imbVeles" >
//
...
namespace imbWEM.Core.crawler.rules.layerRules
{
    ...
    public class layerNeedlePRule:layerDistributionPassiveRuleBase
    {
        public layerNeedlePRule(string __needles, int __layerID, ISpiderEvaluatorBase __parent, int __layerID2 = -1) 
            : base("Needle Test", "Tests Target tokens against the specified needles [{0}], if any token is matched sets layerID to [{1}], otherwise to [{2}].", __layerID, __parent, __layerID2)
        {
            description = string.Format(description, __needles, layerID, layer2ID);
            needles.AddRange(__needles.SplitSmart(",","",true));
        }


        /// <summary> </summary>
        public List<string> needles { get; protected set; } = new List<string>();
